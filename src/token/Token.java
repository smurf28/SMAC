...
/**
 * A class for the tokens returned by a tokenizer. A token is an atomic unit
 * read from the user input. YOU SHOULD NOT CHANGE THIS FILE
 */
public class Token {
	...
	/**
	 * if this is a number token, returns the number as a double, throws a
	 * TokenException otherwise
	 */
	...
	/**
	 * if this is a string token, returns the string as a String, throws a
	 * TokenException otherwise
	 */
	...
	/**
	 * if this is an identifier token, returns the identifier as a String,
	 * throws a TokenException otherwise
	 */
	...
	/**
	 * if this is a delimiter token, returns the delimiter as a String, throws a
	 * TokenException otherwise
	 */
	...
	/**
	 * if this is an operator token, returns the operator as a String, throws a
	 * TokenException otherwise
	 */
	...
	/**
	 * returns a string representation of the token this
	 */
	...
	private enum TokenType {
		NUMBER, IDENTIFIER, STRING, EQUAL, PLUS, MINUS, UNARYMINUS, TIMES, DIVIDE, POWER, OPENPAR, CLOSEPAR, COMMA, SIN, COS, TAN
	}

	private TokenType type; // the type of the token
	private double number; // the double value (when the token is a number)
	private String string; // the String value (when the token is an identifier
							// or a string)
	...
	private static final Token sinToken = new Token(TokenType.SIN);
	private static final Token cosToken = new Token(TokenType.COS);
	private static final Token tanToken = new Token(TokenType.TAN);
	...
	public static Token makeSIN() {
		return sinToken;
	}
	
	public static Token makeCOS() {
		return cosToken;
	}
	
	public static Token makeTAN() {
		return tanToken;
	}
}
